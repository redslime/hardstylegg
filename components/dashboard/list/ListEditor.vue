...
<template>
  <div class="flex items-center gap-2 text-4xl font-bold mb-4">
    <span class="text-primary"><component :is="icons[list.icon ?? 'ListNumberedIcon']" class="size-8 object-cover" /></span>
    <div class="cursor-pointer" @click="editName = true" v-if="!editName">
      {{ list.name }}
    </div>
  ...
  <div class="flex gap-2">
    <fieldset class="fieldset w-96">
      ...
      <textarea class="textarea" maxlength="2048" v-model="list.description"></textarea>
    </fieldset>
    ...
    <fieldset class="fieldset w-96">
      ...
      <div class="flex gap-2 items-center" @click="iconModal?.showModal()">
        <component :is="icons[list.icon ?? 'ListNumberedIcon']" class="text-primary size-10 cursor-pointer object-cover" />
        <p class="opacity-80">(click to change)</p>
      </div>
    </fieldset>
  </div>

  <div class="bg-base-200 p-4 border border-white/10 shadow-md rounded-md mt-6 relative pt-10 max-w-7xl">
        ...
        >
          <template #item="{ element, index }">
            <div class="relative cursor-grab">
              ...
              <CompactArtistItemEditor v-model:item="element.item" v-model:context="element.context" v-else />
            </div>
          </template>
        ...
        >
          <template #item="{ element, index }">
            <div class="relative cursor-grab">
              ...
              <CompactTrackItemEditor v-model:item="element.item" v-model:context="element.context" @remove="removeItem(element)" v-else />
            </div>
          </template>
